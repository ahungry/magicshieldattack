...
func load_game():
	...
	while not save_game.eof_reached():
		var line = Dictionary()
		line.parse_json(save_game.get_line())
		if not TYPE_DICTIONARY == typeof(line) or not line.has("username") or not line["username"]:
			continue
...
func save_game(un):
	var save_game = File.new()
	var json = {"username": un}
	save_game.open('user://savegame.save', File.WRITE)
	save_game.store_line(json.to_json())
	save_game.close()
