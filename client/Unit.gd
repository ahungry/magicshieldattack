...
func set_stance(stance):
	if stance == "move":
		$StanceMove.visible = true
		$StanceMagic.visible = false
		$StanceShield.visible = false
		$StanceAttack.visible = false
	if stance == "magic":
		$StanceMove.visible = false
		$StanceMagic.visible = true
		$StanceShield.visible = false
		$StanceAttack.visible = false
	if stance == "shield":
		$StanceMove.visible = false
		$StanceMagic.visible = false
		$StanceShield.visible = true
		$StanceAttack.visible = false
	if stance == "attack":
		$StanceMove.visible = false
		$StanceMagic.visible = false
		$StanceShield.visible = false
		$StanceAttack.visible = true
...
func animation_event(which, x, y, aegb, face_dir):
	...
	ae_goback = aegb
	$Tween.interpolate_property(self, 'position', position, destination, ae_speed,
								Tween.TRANS_QUAD, Tween.EASE_IN_OUT)
	$Tween.start()
